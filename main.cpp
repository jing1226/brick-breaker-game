...
const char* SAVE_FILE = "breakout_save.dat";
...
struct SaveGameData {
    ...
    uint8_t running;
    uint8_t gameOver;
    uint8_t difficultySpeedIncrease;
    uint8_t ballTouchedPaddle;
...
const int SPATIAL_GRID_WIDTH = 4;   // 网格分割数（宽）
const int SPATIAL_GRID_HEIGHT = 4;  // 网格分割数（高）
...
bool saveGameToFile() {
    ...
    saveData.padLeftX = paddlePosition.x;
    saveData.padRightX = paddleRightX;
    saveData.ballX = ballPosition.x;
    saveData.ballY = ballPosition.y;
    saveData.velX = ballVelocity.x;
    saveData.velY = ballVelocity.y;
    saveData.score = currentScore;
    saveData.lives = lives;
    saveData.brickRows = brickRows;
    saveData.selectedDifficulty = selectedDifficulty;
    saveData.running = (uint8_t)(gameStarted ? 1 : 0);
    saveData.gameOver = (uint8_t)(gameOver ? 1 : 0);
    saveData.difficultySpeedIncrease = (uint8_t)(difficultySpeedIncrease ? 1 : 0);
    saveData.ballTouchedPaddle = (uint8_t)(ballTouchedPaddle ? 1 : 0);
    saveData.currentBallSpeed = currentBallSpeed;
    strncpy(saveData.username, username, sizeof(saveData.username) - 1);
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 14; j++) {
            saveData.bricks[i][j] = bricks[i][j];
        }
    }
...
bool loadGameFromFile() {
    ...
    if (!f) return false;
    SaveGameData saveData;
    if (fread(&saveData, sizeof(saveData), 1, f) != 1) {
        fclose(f);
        return false;
    }
    fclose(f);

    paddlePosition.x = saveData.padLeftX;
    paddleRightX = saveData.padRightX;
    ballPosition.x = saveData.ballX;
    ballPosition.y = saveData.ballY;
    ballVelocity.x = saveData.velX;
    ballVelocity.y = saveData.velY;
    currentScore = saveData.score;
    lives = saveData.lives;
    brickRows = saveData.brickRows;
    selectedDifficulty = saveData.selectedDifficulty;
    gameStarted = saveData.running != 0;
    gameOver = saveData.gameOver != 0;
    difficultySpeedIncrease = saveData.difficultySpeedIncrease != 0;
    ballTouchedPaddle = saveData.ballTouchedPaddle != 0;
    currentBallSpeed = saveData.currentBallSpeed;
    strncpy(username, saveData.username, sizeof(username) - 1);
    username[sizeof(username) - 1] = '\0';
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 14; j++) {
            bricks[i][j] = saveData.bricks[i][j];
        }
    }
    if (spatialGrid) {
...
int main(int argc, char** argv) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (menuState == MENU_PLAYING && IsKeyPressed(KEY_F5)) {
            if (saveGameToFile()) {
        ...
        if (menuState == MENU_TITLE) {
            ...
            DrawCenteredText("1P: A/D KEYS", 190, 25, DARKGREEN);
            DrawCenteredText("2P: ARROW KEYS", 230, 25, DARKGREEN);
            DrawCenteredText("PRESS N TO START NEW GAME", 310, 25, DARKGRAY);
            DrawCenteredText("PRESS L TO LOAD SAVE", 350, 25, DARKGRAY);
            DrawCenteredText("PRESS ENTER TO ENTER USERNAME AFTER NEW GAME", 390, 20, DARKGRAY);
            if (!saveMessage.empty()) {
                DrawText(saveMessage.c_str(), 10, 560, 20, ORANGE);
            }
        } else if (menuState == MENU_USERNAME) {
        ...
        DrawText("PRESS K TO LOAD ASSET", 10, 125, 20, DARKBLUE);
        DrawText("PRESS F5 TO SAVE GAME", 10, 145, 20, DARKBLUE);
        if (!saveMessage.empty()) {
            DrawText(saveMessage.c_str(), 10, 170, 20, ORANGE);
        }
        if (loading) {
            DrawCenteredText("LOADING ASSET...", h/2, 40, PURPLE);
        } else if (finished) {
            DrawText("LOAD COMPLETE! BRICKS TURN GREEN.", 10, 190, 20, GREEN);
        }
